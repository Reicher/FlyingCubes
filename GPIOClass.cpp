...
int GPIOClass::setdir_gpio(string dir)
{
  ...
  if(setdirgpio < 0)
    {
      cout << "OPERATION FAILED: Unable to set dir" << endl;
      return -1;
    }
  setdirgpio << dir;
  setdirgpio.close();
...
int GPIOClass::setval_gpio(string val)
{
  ...
  if(setvalgpio < 0)
    {
      cout << "OPERATION FAILED: Unable to set the value of gpio" << endl;
      return -1;
    }
  setvalgpio << val;
  setvalgpio.close();
...
int GPIOClass::getval_gpio(string& val)
{
  ...
  if(getvalgpio < 0)
    {
      cout << "OPERATION FAILED: Unable to get value og GPIO" << endl;
      return -1;
    }
  getvalgpio >> val;
